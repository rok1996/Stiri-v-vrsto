...
void glavnookno::setPolje2(int i){/*
  QWidget *w = (QWidget*)ui->igralnaP->itemAtPosition(x, y+1)->widget();
  if(i == 1)
    w->setStyleSheet("border-image: url(:/img/tileR.png);");
  else if(i == 2)
    ...
    for(int ii=0;ii<=i;ii++){
        for(int jj=0;jj<7;jj++){
            if(igra->polje[ii][jj]!=0){
                QWidget *w=(QWidget*)ui->igralnaP->itemAtPosition(ii,jj)->widget();
                w->setStyleSheet("border-image: url(:/img/tile.png);\nbackground-color: rgba(255, 255, 255, 200);");
                igra->polje[ii][jj]=0;
                QMessageBox msg;
                msg.setText("i: "+QString::number(ii)+" j: "+QString::number(jj)+"\n");
                msg.exec();
            }
        }
    }
